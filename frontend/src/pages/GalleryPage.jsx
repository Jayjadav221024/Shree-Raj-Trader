...
import { galleryPhotos, companyInfo } from '../data/siteData';
...
/* Captions describe only what is visible in each photograph. The tag is a
   ...
   something the photographs establish. Both live in siteData so the Website
   Editor can change them; this module only decides the page layout.

   Physical sheets of the book: sheet N's front is the right-hand page of spread
   N, its back is the left-hand page of spread N+1. The layout is fixed at five
   sheets, so exactly seven photographs are consumed. */
const buildSheets = () => [
  {
    front: { kind: 'cover' },
...
const TOTAL_SHEETS = 5;               // 5 sheets => spread indices 0..5
...
export default function GalleryPage() {
  ...
  const header = copy['gallery.header'];
