...
import { Category, Product, Testimonial, FAQ, Blog } from './models/WebsiteContent';
...
export const seedDatabase = async () => {
  try {
    ...
    // 1. Seed Menu Groups
    let setupGroup = await MenuGroup.findOne({ name: 'Setup' });
    if (!setupGroup) {
      setupGroup = await MenuGroup.create({ name: 'Setup', order: 1, isActive: true });
      console.log('[Seed] Created "Setup" MenuGroup');
    }

    let cmsGroup = await MenuGroup.findOne({ name: 'CMS' });
    if (!cmsGroup) {
      cmsGroup = await MenuGroup.create({ name: 'CMS', order: 2, isActive: true });
      console.log('[Seed] Created "CMS" MenuGroup');
    }

    let websiteGroup = await MenuGroup.findOne({ name: 'Website' });
    if (!websiteGroup) {
      websiteGroup = await MenuGroup.create({ name: 'Website', order: 3, isActive: true });
    ...
    await MenuMaster.deleteMany({ route: { $in: unusedRoutes } });
    await MenuGroup.deleteOne({ name: 'Master' });

    // Prepend /admin to all existing menus in database
    const existingDbMenus = await MenuMaster.find();
    ...
    const seededMenus: any[] = [];
    for (const item of menuItems) {
      let menu = await MenuMaster.findOne({ route: item.route });
      if (!menu) {
    ...
    // 3. Seed Roles
    let superAdminRole = await RoleMaster.findOne({ name: 'Super Admin' });
    if (!superAdminRole) {
      superAdminRole = await RoleMaster.create({
        name: 'Super Admin',
    ...
    let editorRole = await RoleMaster.findOne({ name: 'Editor' });
    if (!editorRole) {
      editorRole = await RoleMaster.create({
        name: 'Editor',
    ...
    let viewerRole = await RoleMaster.findOne({ name: 'Viewer' });
    if (!viewerRole) {
      viewerRole = await RoleMaster.create({
        name: 'Viewer',
    ...
    let superAdminPerms = await RolePermission.findOne({ roleId: superAdminRole._id });
    if (!superAdminPerms) {
      superAdminPerms = new RolePermission({
        roleId: superAdminRole._id,
        permissions: []
      });
    }

    const existingPermsMap = new Map(superAdminPerms.permissions.map((p: any) => [p.menuId.toString(), p]));
    ...
    const syncRolePermissions = async (
      roleId: any,
      roleLabel: string,
      grant: { canView: boolean; canCreate: boolean; canEdit: boolean; canDelete: boolean }
    ) => {
      let rolePerms = await RolePermission.findOne({ roleId });
      if (!rolePerms) {
        rolePerms = new RolePermission({ roleId, permissions: [] });
      }
      const known = new Set(rolePerms.permissions.map((p: any) => p.menuId.toString()));
      let changed = false;
      for (const m of seededMenus) {
    ...
    let adminUser = await AdminUser.findOne({ email: 'admin@shreeraj.com' });
    if (!adminUser) {
